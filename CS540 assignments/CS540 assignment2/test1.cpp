///
/// Created by macbook on 2020/10/28.
///
/*
 * Run with
 *
...
///*
    {
        ...
        for (auto &e : map) {
            //// sorted by key, so Dave -> Jane -> John -> Mary
            print(e);
        }
        ...
        it--; /// it now points to penultimate element. (penultimate, 2th count start the end)
        map.erase(it);
        ...
        /// Test iterator validity.
        {
            ...
            auto tmp(*b); /// Save, so we can re-insert.
...
///*
    {
        ...
        map.insert(p5);
        /// Should print "0 1 2 3 4".
        /// Shuold be the same as last print
        /// 0, 000; 1, 111; 2, 222; 3, 333; 4, 444;
        for (auto it = map.begin(); it != map.end(); ++it) {
            print(*it);
        }
        ...
        /// Erase the first element. (0,000)
        map.erase(map.begin());

        /// Erase the new first element. (1,111)
        map.erase(map.begin());

        /// Erase the element in the end.(4,444)
        map.erase(--map.end());
        /// Should print "2 3".
        for (auto &e : map) {
            print(e);
...
traverse2(int level) {
    ...
    /// insert four random element and print them
    for (int i = 0; i < 4; i++) {
        char name[30];
