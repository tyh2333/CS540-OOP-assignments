...
void my_test(Deque_MyClass deq) {
    /// my print : to print all elements in the deque
    for (int i = 0; i < deq.d_size; i++) {
...
main() {
    ...
    {
        Deque_MyClass deq;
        Deque_MyClass_ctor(&deq, MyClass_less_by_id); /// sort less by id
...
//        deq.pop_front(&deq);
        ...
        /// debug : my extra test part
/*
        deq.push_back(&deq, MyClass{14, "name1"});
        deq.push_back(&deq, MyClass{15, "name2"});
        deq.push_back(&deq, MyClass{16, "name3"});
        deq.push_front(&deq, MyClass{17, "name4"});
        deq.push_back(&deq, MyClass{18, "name5"});
        deq.pop_back(&deq);
        deq.push_back(&deq, MyClass{14, "name6"});
        deq.push_back(&deq, MyClass{15, "name7"});
        deq.push_back(&deq, MyClass{16, "name8"});
        ///  debug : do my_test to get the member in the deque
//        my_test(deq);
    */
        ///  debug : my extra test part


        /// test for front(), back(), size()
//        /*
        MyClass_print(&deq.front(&deq));
        MyClass_print(&deq.back(&deq));
        assert(deq.front(&deq).id == -1); /// front should be -1
        assert(deq.back(&deq).id == 3); /// back should be 3
...
//        /*
        /// Change via at().
        assert(deq.at(&deq, 0).id == 0); /// check if array[0] = 0
        deq.at(&deq, 0).id = 100;  /// change array[0] = 100;
        assert(deq.at(&deq, 0).id == 100); /// check again
        /// Change via front().
        assert(deq.front(&deq).id == 100); /// check front
        deq.front(&deq).id = 0;
        assert(deq.front(&deq).id == 0); /// check again
...
//     */
    ...
    //// Test sort: You must be able to work with the correct less-than function.
//    /*
    {
        Deque_int deq;
        Deque_int_ctor(&deq, int_less);
